...
**The one known, deliberately unresolved gap:** a fall-protection/scaffolding query still bypasses the threshold (score 8.58), because it coincidentally shares a genuinely rare word — "requirement" (singular), appearing in exactly 1 of 185 chunks, identical corpus-wide rarity to `V6001176` — with an unrelated fire-extinguisher clause. This isn't a tunable-parameter miss: BM25 has no way to distinguish "rare word reflecting real relevance" from "rare word shared by coincidence," since that's a semantic judgment, not a statistical one — the same kind of blind spot hybrid retrieval exists to fix on dense search's side, now showing up as lexical search's own mirror-image limitation. See `BM25LexicalIndex.search()`'s docstring for the full reasoning, and `bm25_min_score`'s comment in `config.py` for the calibration data behind the current floor value. A deeper fix (routing lexical-only hits through the reranker's actual semantic judgment instead of a score floor) is noted there as a candidate follow-up, not yet built.
...
- [ ] Route lexical-only hits through the reranker's own semantic judgment instead of a raw BM25 score floor — candidate follow-up to the remaining gap above, only helps when `RERANK_ENABLED=true`
